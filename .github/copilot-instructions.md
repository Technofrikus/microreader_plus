...
| `BmpSleepConverter.h/.cpp` | Converts BMP files to MGR2 format (2bpp, 4-level grayscale) for sleep screen images. Supports 1bpp, 2bpp, 4bpp, 8bpp (indexed), 16bpp (RGB565/BGR555), 24bpp, and 32bpp (BI_RGB/BI_BITFIELDS) formats. Scales images to 800×480 with portrait rotation support. Auto-size mode (out_w=0, out_h=0) preserves source aspect ratio after rotation, preventing white borders on non-standard display sizes like X3 (792×528). |
